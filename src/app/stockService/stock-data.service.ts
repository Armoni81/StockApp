...
export class StockDataService {
  ...
  getVolAndChangeData(arr: []){
    ...
    for(let i = 0 ; i < arr.length ; i++){
      ...
      this.http.get<{code: string, change: number, change_p:number, volume:number}>(`https://eodhd.com/api/real-time/${arr[i]}.US?api_token=67908244556db6.70380137&fmt=json`).subscribe((data) => {
